...
const ChannelInner = ({setIsEditing}) => {
    ...
    const overrideSubmitHandler = (message, file) => {
        console.log( message );
        console.log("File type is " + file);

        let URL = message.attachments.length > 0 ?
        message.attachments[0].image_url ?
        message.attachments[0].image_url :
        message.attachments[0].asset_url :
        ''

        let encryptedFile = encFile(URL);

        let Message = {
            text: message.text + "" + encryptedFile,
